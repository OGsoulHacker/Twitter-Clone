...
import { collection, doc, setDoc, updateDoc, deleteDoc, query, where, getDoc, getDocs, onSnapshot } from 'https://www.gstatic.com/firebasejs/10.12.3/firebase-firestore.js'
...
onAuthStateChanged(auth, (user) => {
  if (user) {
    console.log("Got user1: ", user.email);
...
function btnsEffects(id) {
  ...
  //effects on post button (post-btns)
  ...
  postBtns.forEach((btn) => {
    ...
    btn.addEventListener("mouseover", () => {
      ...
      } else if (btn.className == "edit-btn") {
        btnImgBox.style.backgroundColor = "rgba(29, 161, 242, 0.1)";
        btnSvgPath.style.fill = "rgb(29, 161, 242)";
        btnSvgPath1.style.fill = "rgb(29, 161, 242)";
    ...
    btn.addEventListener("mouseout", () => {
      ...
      if (btn.className == "comment-btn" || btn.className == "retweet-btn" || btn.className == "like-btn") {
...
function createPost(id, content, time, postUser) {

  // const auth = getAuth();
  const user = auth.currentUser;
  console.log("Got user2: ", user.email);
  
  const userEmail = user.email;
  console.log("Got user3: ", userEmail);
  ...
  <div class="post-middle">
    <div class="post-header">
      <h2>${postUser.firstName} ${postUser.lastName}</h2>
      <span class="username">${postUser.username}</span>
      <span class="dot">.</span>
...
async function editPost(id) {
  ...
  // Display current content in a textarea for editing
  const editText = prompt("Edit your tweet:", postContent);
  if (editText !== null) {
    ...
    const postContentDiv = document.getElementById(`post-content-${id}`);
    postContentDiv.innerHTML = editText;
  }
}
  ...
  form.addEventListener("submit", async (e) => {
    ...
    try {
      ...
      querySnapshot.forEach((doc) => {
        ...
        // });
        console.log("Post %d written with ID: %d", postId, docRef.id);
      });
...
onSnapshot(postsQuery, (snapshot) => {
  ...
  changes.forEach((change) => {
    if (change.type == "added") {
      ...
      const postUser = change.doc.data().user;
      ...
      createPost(postId, content, time, postUser);
    }
  });
});
